...
-(id) initFromXMLFile:(NSString *)_xmlfile {
    // Init with a blank png, so that it will work under batchNodes
    // NOTE: Suffix is used in case you have different spriteSheets
...
- (void) initProperties
{
    // BEWARE! In order to avoid childrenTable to retain objects (CCNODES) and prevening conflict in dealloc
...
-(void)addDebugGraphics{
    // Show Anchor Point (actually (0,0) of root node
    CCSprite *apSprite;
    CGSize size = CGSizeMake(10, 10);
    
    // NOTA IMPORTANTE!
    // This method is no longer valid because FTCCharacters are now added to a batchNode, only being able to draw graphics associated to that batchNode
...
//    free(buffer);
    ...
    // Show rectangle used by that character
//    tex = [CCTexture2D new];
//    [tex initWithData:buffer pixelFormat:kCCTexture2DPixelFormat_RGB888 pixelsWide:1 pixelsHigh:1 contentSize:size];
//    apSprite = [CCSprite node];

...
+(BOOL) parseSheetXML:(NSString *)_xmlfile toCharacter:(FTCCharacter *)_character
{
    ...
    // Set device factor, used to modify all transform values if using different sizes
    float pf = 1.0f; // NOTE: Must be changed to use device factor (ie: 1, 0.5, 0.25)
    ...
    do {
        ...
        float nAX           = [[TBXML valueOfAttributeNamed:@"registrationPointX" forElement:_texture] floatValue]*pf;  // 
        float nAY           = -([[TBXML valueOfAttributeNamed:@"registrationPointY" forElement:_texture] floatValue]*pf);  // 
        ...
        // Add to the shared info of that sprite
        ...
        // Assign dictionary with animations
...
// Parses and preloads in FTCSpriteAnimations all animations of provided character (_xmlfile)
        ...
        do {
            ...
            if (_frameInfo) {
                do {
                    ...
                    /*
                     * Calculates contentSize
                     */
                    // Save necessary information from sprites to be able to calc later
            ...
            // Optimize: Must create a dictionary instead of using sprite's one
